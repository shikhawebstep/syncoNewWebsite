...
const StepStudent = ({ index, title, totalStudents, errors, setErrors, venueClasses }) => {
  ...
  console.log('venueClasses', venueClasses);
      ...
      {(totalStudents === 1 || open) && (
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
          ...
          <div>
            ...
            {index === 0 ? (
              // For first student - show selected class (read-only if auto-selected)
              <input
                readOnly
                value={data.class || ""}
              ...
              // For other students - dropdown with NO default selection
              <select
                value={data.classScheduleId || ""} // Always empty unless user selects
                onChange={(e) => {
                  const selectedId = e.target.value;
                  const selectedClass = venueClasses?.find(c =>
                  ...
                  if (selectedClass) {
                    ...
                    // Clear selection if "Select class" is chosen
                    updateStudent(index, {
                      ...data,
                      classScheduleId: "",
                      class: "",
...
export default function StepStudents({ classDetails, venueClasses }) {
  ...
  const [errors, setErrors] = useState(() => Array(childrenCount || 0).fill(null).map(() => ({})));

  console.log('students', students);

  // Update only the useEffect in StepStudents component
  useEffect(() => {
    setStudents((prev) => {
      const updated = [...prev];

      // Ensure correct length
      while (updated.length < (childrenCount || 0)) {
        updated.push({});
      }

      return updated.slice(0, childrenCount || 0).map((student, index) => {
        // 👉 ONLY for first student
        if (index === 0) {
          // If already selected, keep it
          if (student.classScheduleId) return student;

          // Otherwise assign first available class
          if (venueClasses && venueClasses.length > 0) {
            const firstClass = venueClasses[0];
            ...
            return {
              ...
              class:
                firstClass?.level
                  ? `${firstClass.className} (${firstClass.level})`
                  : firstClass.className, time: `${firstClass.startTime} - ${firstClass.endTime}`,
            };
          }
        }

        // 👉 ALL OTHER STUDENTS → no default class
        return {
          ...student,
    ...
    // Sync errors length
    setErrors((prevE) => {
      if ((prevE?.length || 0) !== (childrenCount || 0)) {
        return Array(childrenCount || 0).fill(null).map(() => ({}));
      }
      return prevE || [];
    });
  }, [childrenCount, venueClasses, setStudents]);
  ...
  return (
    <div className="xl:max-w-[676px] mx-auto md:p-6 pt-0 poppins">
