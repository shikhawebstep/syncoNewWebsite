import React from "react";
import bannerImg from "/assets/whyBrazilBg.png";
const TrainingKit = () => {
    return (
        <section
            className="relative  flex bg-contain bg-no-repeat items-end bg-[#fff]  bg-top py-20 lg:min-h-[800px]"

        >
            <div className="container z-999 mx-auto">
                {/* Heading */}
                <div className="max-w-[1210px] m-auto px-5 md:px-0">
                    <h2 className="md:text-center items-center flex justify-center gap-6 text-[#042C89] text-[52px] md:text-[56px] font-normal recline mb-12">
                        The Official Training Kit

                    </h2>
                    <p className="text-[#5F5F6D] pt-5 md:py-0 md:text-[20px] poppins md:text-center" >The items below are compulsory for <br className="block md:hidden" /> students attending our classes.</p>
                    {/* Content */}

                    <div className="border-3 mt-15  rounded-[20px] py-15 border-white bg-[#FDFDFF]  shadow-[0_0px_12px_rgba(0,0,0,0.12)]">
                        <h2 className="text-center items-center flex justify-center gap-6 text-[#00A6E3] text-[52px] md:text-[56px] font-normal permanent-marker mb-12">
                            Kids Collection
                        </h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 max-w-[840px] m-auto lg:grid-cols-3 gap-10 mt-10 leading-relaxed place-items-center">

                            <div className="text-center space-y-5">
                                <img
                                    src="/assets/SSSTshirt.png"
                                    alt="SSS Kids T-Shirt"
                                    className="mx-auto max-w-[200px]"
                                />
                                <span className="text-[20px] recline text-[#042C89]">
                                    SSS Kids T-Shirt
                                </span>
                            </div>

                            <div className="text-center space-y-5">
                                <img
                                    src="/assets/SSSDri-FITShorts.png"
                                    alt="SSS Dri-FIT Shorts"
                                    className="mx-auto max-w-[200px]"
                                />
                                <span className="text-[20px] recline text-[#042C89]">
                                    SSS Dri-FIT Shorts
                                </span>
                            </div>

                            <div className="text-center space-y-5">
                                <img
                                    src="/assets/SSSSocks.png"
                                    alt="SSS Classic Socks"
                                    className="mx-auto max-w-[200px]"
                                />
                                <span className="text-[20px] recline text-[#042C89]">
                                    SSS CLASSIC II Socks
                                </span>
                            </div>

                        </div>

                        <button className="bg-[#FFDE14] mx-auto mt-15 text-[#042C89] flex justify-center items-center rounded-3xl p-2 px-4 text-[18px] gap-2 font-semibold "> <img src="/assets/cartIcon.png" className="w-4 h-4" alt="" /> <a href="" className="recline"> Buy Today!</a></button>

                    </div>
                    <div className="border-3 mt-10 rounded-[20px] py-15 border-white bg-[#FDFDFF]  shadow-[0_0px_12px_rgba(0,0,0,0.12)]">
                        <h2 className="text-center items-center flex justify-center gap-6 text-[#0DD180] text-[52px] md:text-[56px] font-normal permanent-marker mb-12">
                            Coaches Collection
                        </h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 max-w-[840px] m-auto lg:grid-cols-3 gap-10 mt-10 leading-relaxed place-items-center">

                            <div className="block text-center space-y-5">
                                <img src="/assets/Coaches T-Shirt.png" alt="" />
                                <span className="text-[20px] recline  text-[#042C89] text-center">Coaches T-Shirt</span>
                            </div>
                            <div className="block text-center space-y-5">
                                <img src="/assets/Coaches Mid-Layer.png" alt="" />
                                <span className="text-[20px] recline  text-[#042C89] text-center">Coaches Mid-Layer</span>
                            </div>
                            <div className="block text-center space-y-5">
                                <img src="/assets/Coaches Winter Jacket.png" alt="" />
                                <span className="text-[20px] recline  text-[#042C89] text-center">Coaches Winter Jacket</span>
                            </div>
                        </div>
                        <button className="bg-[#FFDE14] mx-auto mt-15 text-[#042C89] flex justify-center items-center rounded-3xl p-2 px-4 text-[18px] gap-2 font-semibold "> <img src="/assets/cartIcon.png" className="w-4 h-4" alt="" /> <a href="" className="recline"> Buy Today!</a></button>


                    </div>

                </div>
            </div>


        </section>
    );
};

export default TrainingKit;
